...
- **Operator authorisation** — gates coordinator **enrollment** and the
  ...
  *approving* operator's confirmation of the coordinator's key in
  ...
  on the admin service token). Design lives in [`mint.md`](mint.md)
  (the enrollment flow and admin plane) and
  [`auth-service.md`](auth-service.md) (the
...
service in [`auth-service.md`](auth-service.md) and the
[`mint`](mint.md) enrollment flow.**
...
(see [`mint.md`](mint.md)) removes that: to mutate S3 a
...
in [`auth-service.md`](auth-service.md).

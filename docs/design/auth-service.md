...
[`auth-model.md`](auth-model.md) — **operator authority is
...
- **Mint admin plane** (where *approve* lives) — the admin service token carries a TPC, so every
  ...
  see [`mint.md`](mint.md) § *Operator authorization*).
...
[`mint.md`](mint.md#reference-client--demo)). The
...
([`mint.md`](mint.md) § *Enrollment*): the *enrolling*
...
`CID` wraps `r_adm` (see [`mint.md`](mint.md) § *Operator
...
they are plain key-bound service tokens (`docs/design/mint.md` § *Credential
macaroon & lifecycle*).
...
attenuation are specified in [`mint.md`](mint.md) §
...
rotation — see [`mint.md`](mint.md) § *Root-key
rotation*.)
...
([`mint.md`](mint.md) § *Enrollment*): `POST /v1/enroll`

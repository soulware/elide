...
- **Volume roles are attested, per volume.** `volume-rw` / `volume-ro` bind the
  ...
  (`docs/design/mint-volume-attestation.md`). Control-plane roles (`coord-ro`,
...
**Why not the per-purpose split (Split A).** `docs/design/iam-key-model.md`'s
...
(`docs/plans/list-elimination-plan.md` § *Reconcile/repair without LIST*).
...
prefix, each authorized by lineage (`docs/design/mint-volume-attestation.md`):
...
1. **Volume process reads** — coordinator vends the keypair to the volume
   ...
   peer's local bytes (`docs/design/peer-segment-fetch.md`).
...
`docs/design/iam-key-model.md` § *Per-volume scoping for writes (rejected)*
...
(`docs/design/iam-key-model.md` § *IAM-layer invariants*). `meta/*` does not

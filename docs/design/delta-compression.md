...
A never-derived volume re-attempts derivation at every promote take, so a filesystem formatted mid-session gains awareness within one drain tick of its first writes (on a blank device each attempt costs one read of LBA 0 — the superblock-magic probe fails before deeper parsing). A first-ever successful parse **flips the window live**: an activation ULID is minted above every existing segment and persisted alongside the ranges (`[journal] activation`), and classification everywhere is `window.contains(lba) && segment_ulid >= activation` — a pure function of the entry's LBA and its containing segment's ULID, so the live index, the drift checker, readonly opens, and the coordinator's GC rebuild all agree on the stamps of pre-flip segments (below the marker, stamped under the empty window) without any of them replaying history. The next open reclassifies uniformly under the plain window and clears the marker; pre-flip journal-window entries are journal churn, so they die within a wrap either way. A volume whose window was already derived keeps the session-stable rule: a changed window (reformat) persists at open and takes effect at the next open, and an affirmative "not ext4" parse at open clears the section — the volume reverts to never-derived and the poll resumes, so a device reformatted away from ext4 (and later, perhaps, back) tracks reality instead of keeping a stale window. The clear requires written content: an unreadable device keeps the stored window, and so does an unwritten superblock block (zeros from the void on a blank or discarded device are absence of evidence). The section always holds the last affirmative answer.
...
**Segment segregation.** A promote whose epoch touched journal LBAs writes a segment pair: the stable share under the first ULID, the journal-window share under a second, higher ULID. jbd2 is a circular log — the head-run written in one epoch is overwritten by a later wrap, so a journal segment's claims die together and the whole file is reaped instead of folding live neighbours out of mixed segments. Formation establishes the split and nothing downstream maintains it: GC's bin-pack has no journal concept and dissolves the pair on the first pass that buckets a journal segment ([gc-journal-segregation.md](gc-journal-segregation.md)). The ULID order is load-bearing: canonicals live in the data segment, so a journal entry minted as a DedupRef points backward as DedupRefs must. An epoch with no journal writes forms a single segment exactly as before.

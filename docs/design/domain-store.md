...
[`mint.md`](mint.md) § *Coordinator store architecture*.
...
The post-snapshot delta from `docs/design/segment-index.md`. Sole writer
...
No CAS — the document is justified in `docs/design/segment-index.md`.
...
4. `VolumeData::snapshots()` — manifest publish / fetch and the
   ...
   #414).* Verb shape:
   - `put_manifest(snap, Bytes)` for in-memory builds (owner-signed
     empty handoffs, the forced-claim basis copy).
   - `put_manifest_from_file(snap, &Path)` for disk-to-S3 uploads
     (regular drain seal path). Reads the file
...
- **GC compaction segment writes.** Multi-segment compaction output
  ...
  proposed is fine, but the *plan* object (`docs/design/gc-plan-handoff.md`)

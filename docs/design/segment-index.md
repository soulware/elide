...
P3 (folding in P4) of [`../plans/list-elimination-plan.md`](../plans/list-elimination-plan.md):
...
[`events/<name>/HEAD`](volume-event-log.md): both name the
...
- **`Superseded{input_ulid, output_ulid, since}`** — GC published
  ...
  (`max(inputs).increment()`, `docs/design/gc-ulid-ordering.md`), not
...
- The **manifest spine has no reconcile to add** — it is signed and
  durable on its own (P1/P2, `docs/design/volume-event-log.md`).

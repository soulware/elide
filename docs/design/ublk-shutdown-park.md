...
- **Unprivileged tier interaction.** With `UBLK_F_UNPRIVILEGED_DEV` (multi-tenant tier per `docs/design/ublk-transport.md`), the per-tenant uid owns the device. Does the operator-run reconciliation sweep need additional permissions to delete a tenant's parked device? Likely yes — sweep belongs to a privileged context, not the per-tenant daemon.

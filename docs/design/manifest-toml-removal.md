...
Follow-up to [volume-size-ownership.md](volume-size-ownership.md). With `size` relocated to `NameRecord`, every remaining field on `manifest.toml` was either redundant with an existing surface or migrated onto a signed one, so the file is gone.
...
- `docs/design/volume-size-ownership.md`: flip the "could be dropped entirely as a follow-up" line to "dropped — see [docs/design/manifest-toml-removal.md]" once this lands.

...
- `manifest.toml` no longer carries `size` (and is now gone entirely — see [manifest-toml-removal.md](manifest-toml-removal.md)). Pulled-ancestor skeletons fetch `volume.pub + volume.provenance` only — no `volume.toml`.
...
Dropped entirely — see [manifest-toml-removal.md](manifest-toml-removal.md). OCI source migrates to a signed `oci_source` block on `volume.provenance`; the other three fields go away with no replacement.

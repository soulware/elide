...
- **Interaction with CANONICAL_ONLY demotion for hashes used as dedup/delta sources.** When the rewritten hash H has DedupRef or Delta entries elsewhere pointing at it, GC partial-death demotes H to `CANONICAL_ONLY` (body preserved, no LBA claim) rather than dropping it. This is a clean separation of concerns — H' serves this volume's LBA reads with no aliasing overhead; H serves remote dedup/delta consumers with the full body they hashed against — not a failure mode. But it does change the cost accounting: when H has outbound dedup/delta refs, reclaim adds `sizeof(H')` of new body on top of the preserved H; when H has none, reclaim is essentially free (H is orphaned and GC drops it entirely). The scanner threshold may want to factor in outbound-ref presence so the "worth firing" bar is stricter for hashes whose old body will survive.
  ...
  **Output-shape optimisation.** The same signal drives a per-entry decision inside `execute_reclaim`. When H is a DATA entry and `delta_source_refcount(H) > 0`, the sliced sub-range is re-emitted as a thin Delta with `source_hash = H` rather than a fresh Data/Inline: zstd-dict-compressing a literal substring of H against H itself resolves to a few-hundred-byte dictionary reference, so the output is dramatically smaller. H's body was going to stay on disk regardless (some other live Delta sources from it), so pinning it via our own output costs no additional retention. When `delta_source_refcount(H) == 0`, H would have been orphaned by the reclaim and dropped on the next GC pass; in that case we emit fresh Data so GC can actually reclaim the body. A delta blob larger than the raw sub-range falls back to Data. DedupRef still beats both shapes when the new hash is already canonical.

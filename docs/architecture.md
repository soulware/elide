...
**Writes take the volume mutex directly**, on the ublk queue thread, for the WAL append, the claim staging and the snapshot publish. They do not pass through the actor. A `fua` write takes the WAL's file handle under the mutex and syncs it after releasing, so the device round trips of concurrent FUA writers overlap and land in one host filesystem commit. The handle survives a promote rotating the WAL underneath it, because the old WAL is unlinked only once the segment carrying its bytes is committed.

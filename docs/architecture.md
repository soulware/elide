...
Background promotes that fail (I/O error, disk full) are logged and do not crash the actor — the data is safe in the WAL file, which stays on disk and inside the flush fsync set until a retry promotes it. The next explicit `Flush` or threshold-triggered promote will surface the error.

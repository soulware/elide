...
The `<S>-stop.manifest` files written by `stop` are *checkpoints*, not stable references:

- **Created** by `stop` (writable volumes only). The signed payload is identical to a user snapshot; only the filename differs.
- **Skipped at stop** when a manifest at the target ULID already exists locally — either a user snapshot taken since the last write, or a stop-snapshot from a prior stop that hasn't been cleaned yet. The existing manifest is the basis; republishing would be redundant work.
- **Deleted** by the coordinator's `NotifyVolumeReady` handler once the volume binary signals that `Volume::open` succeeded. Deferring the cleanup until "the daemon has proven the local fork is openable" means a failed spawn (supervisor bug, ublk module missing, etc.) doesn't leave the operator with an unrecoverable name — the S3 basis stays in place until the next `stop` overwrites it. While Live, the cleaned stop-snapshot is gone; while Stopped, exactly one stop-snapshot is in S3.
...
- **Not used** as a fork base. `volume create --from` and other lineage-creating verbs enumerate `by_id/<vol>/snapshots/<ulid>.manifest` only — stop-snapshots are filtered out. Tying a new lineage to ephemeral state would be a footgun.
- **Promoted** by `volume release`'s fast path (writable host giving up the name when nothing has changed since the stop-snapshot): the stop-snapshot is renamed to `<S>.manifest` — local file rename, S3 GET+PUT of the (small) signed bytes to the user-manifest key, DELETE of the stop key, plus a rename of the upload sentinel — before the bucket flips to `Released`. Once promoted, claim-from-other-host has a stable basis to fork from. Release's slow path (work since the last snapshot, or a stale/missing sentinel) bypasses promotion and produces a fresh user manifest the normal way. **Proposed:** `claim --force` needs no promotion on the recovery path: its tail re-own from the dead fork's HEAD covers any segments an unpromoted stop-snapshot pinned. (See [Disaster recovery](#disaster-recovery).)

### Signing-key shadow

The per-volume Ed25519 signing key (`volume.key`) is never uploaded to S3 — it lives only on the host that minted it. The coordinator maintains a local shadow under `data_dir/keys/<vol_ulid>.key`; it is `start`'s possession proof when the local fork directory is gone.

- **Written** at every key-generation site: `volume create` (root), fork creation (`fork_create_op` after `fork_volume_at*`), cross-coordinator claim (`claim::early_rebind`), and `claim --force`. The shadow is 32 raw bytes, identical to the on-disk `volume.key`. A shadow-write failure aborts the mint, so an owned-but-keyless volume cannot arise.
...
**Writes take the volume mutex directly**, on the ublk queue thread, for the WAL append, the claim staging and the snapshot publish. They do not pass through the actor. A `fua` write and a FLUSH op both run through the WAL sync gate (`docs/design/wal-sync-gate.md`) on the ublk thread: a request carries the generation its writes published under, one fdatasync round serves every request whose generation is at or below the round's start, and a round syncs the open WAL plus every rotated WAL whose promote is uncommitted, through handles the volume keeps until the segment commits.
...
The consequence for anyone reasoning about latency: **moving work off the actor does nothing for the guest. What the guest waits for is a critical section, and how many of them run back to back.** A long operation on the worker costs the guest nothing, because the worker holds no lock. A short operation on the actor costs the guest exactly as long as it holds the mutex. When the question is "where should this run", the answer is decided by lock hold time, not by thread.

**A loop of short critical sections costs the guest their sum.** The volume mutex is `std::sync::Mutex`, which hands the lock to whichever thread wins the next acquisition rather than to the longest waiter. An actor loop shaped `lock → work → drop → lock` re-acquires while the writers it just released are still being woken, so the same write loses every round. Measured on the soak rig: a repack apply took the mutex 12 times at 58 ms or less each, and a guest write waited 541.7 ms — 81% of the 667.1 ms the mutex was held at all that window. Splitting one long hold into many short ones therefore moves the guest's wait only as far as the loop yields between them.

Writes bypassing the actor is deliberate. Routing them through the actor's request channel would queue every write behind whatever the actor is doing, so a long apply would delay all of them rather than only those that collide; taking the mutex directly lets a write proceed as soon as it wins an acquisition against the actor's next iteration.
...
3. For each extent:
   ...
   b. Write DATA record with the (optionally compressed) payload to WAL (fsync = durable)
4. Update live LBA map with new (start_LBA, length, extent_hash) entries
```

Durability is at the write log. S3 upload is asynchronous and not on the critical path. The write path performs no extent-index lookup and holds no dedup policy; every record carries its body, so the WAL is self-contained at crash recovery. Dedup is decided at segment formation — see the Dedup section.
...
**Dedup is local and decided at segment formation.** When the WAL is promoted to a pending segment, each entry's hash is checked against the local extent index; an entry whose hash already resolves elsewhere — the local tree, an ancestor, or best-effort another volume on the same host — is minted as a thin DedupRef and its WAL body bytes are dropped from the segment. See the Dedup section below for full scope details.

**Delta compression is decided at segment formation too.** After the exact-hash tier, the promote worker deltas remaining single-block Data entries against same-LBA extents from the latest sealed snapshot (zstd dictionary, kept only when smaller than the stored body; best-effort — a source body not locally present is skipped, never fetched). Local segment bodies therefore contain full extent data (Data entries), delta blobs in the segment's delta region (Delta entries), or no body bytes at all (DedupRef and Zero entries).

**Journal-window entries form their own segment.** A promote whose epoch wrote inside the guest filesystem's jbd2 journal window emits a segment pair: the stable share first, the journal share under a second, higher ULID. Journal LBAs are cyclically overwritten, so a journal segment's claims die together at wrap and GC reaps the file whole instead of folding live neighbours out of a mixed segment. The journal partition never enters the delta tier, and journal runs are excluded from the delta source map. Epochs with no journal writes form a single segment. A DedupRef is resolved via the extent index to a canonical Data entry somewhere reachable — in the same volume (including the same segment), an ancestor, or another local volume under the same dedup scope; a Delta is materialised by decompressing its blob against its source body.
...
**Exact dedup:** two extents with the same BLAKE3 hash are identical. Dedup is detected and applied **at segment formation**: when the WAL is taken into a promote job, each pending Data or Inline entry's hash is checked against the local extent index. An entry whose hash already resolves to another body — a committed canonical, an ancestor's, or an earlier write of the same bytes in the same WAL epoch — is converted to a DedupRef, and the segment write drops its body bytes. The write path itself always appends full DATA records; a duplicate's body sits in the WAL until the next promote thins it, bounded by the flush threshold.
...
**Canonical ownership is "lowest non-journal ULID wins".** When the same hash appears in multiple places, the extent index makes the earliest copy canonical (first-write-wins on the live path, ULID-ascending walk at rebuild) — except that a copy whose LBA falls inside the guest filesystem's jbd2 journal window owns its hash slot only while no stable-LBA copy exists; a non-journal copy displaces it. jbd2 writes a metadata block's journal image before its home-LBA checkpoint, so without the preference the churniest LBAs on the device would own the canonicals that durable metadata DedupRefs against. With it, the home copy owns and the journal copy is the thin DedupRef that dies at wrap. The journal window is derived from the filesystem (`ext4_scan::journal_lba_ranges`) — at open, and for a never-derived volume at every promote take, flipping live with an activation marker when the filesystem was formatted mid-session — and persisted in `volume.toml` (the `[journal]` section: `ranges`, plus `activation` during a flip session) so every rebuild applies the same rule before the filesystem is parseable; see `docs/design/delta-compression.md` § Journal-region awareness.

**DedupRef entries are format-level thin.** When dedup fires at formation, the segment's DedupRef index entry stores `stored_offset = 0` and `stored_length = 0` — it reserves no body space at all. The segment header's `body_length` is the sum of `stored_length` over DATA and INLINE entries only; DedupRef entries contribute zero. This layout is **unified across `pending/`, `cache/`, and S3**: the same segment bytes, the same body size, the same index entry semantics everywhere. There is no "local-disk vs S3" asymmetry; thin DedupRef delivers S3 storage savings directly, without a materialisation or compaction step.
...
- **Extent index:** required for dedup detection at segment formation and for serving all DedupRef reads. The extent index is a hard dependency of the read path, local and S3 alike. This is an explicit trade-off: segment-level self-containment is given up in exchange for a format that is thin everywhere.
...
**Delta compression** is a separate concern from dedup. See [design/delta-compression.md](design/delta-compression.md) for the full design; the summary below covers the format interaction with GC, the extent index, and `lba_referenced_hashes`.

**Source selection** has two production producers. Import-time filemap path-matching across imported snapshots linked by `extent_index` provenance lineage: for a changed file in a child import, the delta source is the same path in the parent import, computed in-process in `elide-import` after pending segments are written but before the volume is promoted. The formation delta tier for live-written volumes: same-LBA sources from the latest sealed snapshot, applied by the promote worker as each WAL becomes a pending segment.
...
4. **Admission within each rebuild pass** runs in two phases. Flush-tier segments (empty inputs table) admit by highest claimant ULID — mint order is write order, so the highest claimant is the newest write. Compaction outputs (recorded inputs table: bare `gc/<ulid>` files and the close pass's repack outputs) admit afterwards, ULID ascending, under their **view horizon** `max(inputs)`: an output overrides claimants at or below the horizon (claims its pass classified) and defers to claimants above it (writes flushed after the classification, whose claims the live apply kept via `insert_consuming_inputs`). The horizon is needed because an output can mint above a flush that landed mid-pass; admitting outputs by bare ULID there would resurrect the stale claim the live apply refused. Bodies with `.staged` siblings are coordinator-staged and not yet volume-re-signed; they are excluded by `collect_gc_applied_segment_files`.
5. Replay the WAL.
...
- **GC ULID assignment:** `gc_checkpoint`'s pre-mint is safe because both `u_gc` and `u_flush` come from the volume's own `UlidMint` under `u_gc < u_flush`, and any subsequent write draws a ULID strictly above `u_flush`. A write that races with compaction therefore lands above `u_gc` and wins at rebuild's oldest-first replay. No locking is required — the ordering is established at mint time, not by serialising the write.
...
- Produced by the self-describing GC handoff protocol: the coordinator creates compacted segments at `gc/<ulid>.staged` (signed with an ephemeral key) with the input ULID list embedded in the segment header. The volume reads `.staged`, derives the apply set from the inputs field, writes a re-signed copy to `gc/<ulid>.tmp`, and renames it to bare `gc/<ulid>` (the atomic commit point). Only after the bare file appears does the coordinator upload it to S3 and trigger `promote_segment` IPC, which writes the resulting `index/` + `cache/` files. The bare file is deleted at the end via `finalize_gc_handoff` IPC. This ensures S3 always receives the volume-signed version.

This protocol keeps the private key (`volume.key`) on the volume host and prevents the coordinator from forging new data.

**Demand-fetched segments are integrity-checked at fetch time, not re-signed.** Each ancestor's segments retain the originating volume's signature; the current volume's `volume.key` is never used to re-sign fetched data. Verification is layered:

- **Signed `.idx` install (prefetch).** When the coordinator pulls an ancestor `.idx` from S3, `prefetch_indexes` calls `verify_segment_bytes` against the appropriate verifying key — own `volume.pub` for the current fork, or the parent's embedded `pubkey` from the child's signed provenance for each walked ancestor — and refuses to write a tampered `.idx` to disk. Subsequent local reads via `read_segment_index` rely on this install-time check; there is no unverified path to disk.
- **Body byte fetch (demand).** Every entry in a range-GET is hash-verified against the signed `.idx` before bytes hit `cache/<id>.body` (`elide-fetch::verify_body_hash` per entry; mismatch rejects the batch and falls through to the next ancestor in the chain).
- **Delta-blob fetch.** Each delta option is verified against its signed `delta_hash` before the blob is written to `cache/<id>.delta`.

Re-signing fetched segments under the current `volume.key` was considered and rejected: the per-ancestor signature chain — anchored by the parent `pubkey` field embedded in each child's signed provenance — already establishes integrity end-to-end. Re-signing would consolidate verification under a single key but would not add any guarantee the chain doesn't already provide, and would force the coordinator to round-trip every fetched body through the volume process for signing. `cache/<id>.body` files therefore remain coordinator-written and unsigned by the local key; their integrity is established at fetch time and not revisited.
...
**WAL promotion (decoupled from writes):** `Volume::write()` only appends to the WAL and updates the in-memory maps — it never touches the segment layer. The actor is responsible for preparing a promote and dispatching it to the worker thread via two mechanisms:

1. **Threshold-triggered:** after sending the write reply, the actor checks `Volume::needs_promote()`. If the WAL has reached the 32 MiB soft cap, it calls `prepare_promote()` and sends a `WorkerJob::Promote` to the worker. The actor's own work is cheap: snapshot CAS tokens, mint a fresh segment ULID, create a fresh WAL file. No fsync. The worker runs the old-WAL `sync_data()` as the first step of the promote, then writes + commits the pending segment, then returns a result to the actor. The actor stays on the select loop the whole time, continuing to process writes onto the fresh WAL.
...
**FLUSH:** `VolumeRequest::Flush` fsyncs the current WAL on the actor and then fsyncs every rotated WAL the promote pipeline still owns — promotes in flight on the worker and failed promotes stashed for retry. Both fsyncs run on the actor thread, so a flush ack is bounded by fsync cost and independent of the worker queue: a promote sitting behind a long close pass adds nothing to flush latency. Crash recovery replays every WAL file it finds in `wal/`, promoting all but the newest, which is what makes a fsynced rotated WAL a durable copy of its epoch. The worker's own `sync_data()` at the start of each promote makes the common case cheap — a flush usually finds the rotated WAL already clean.

Background promotes that fail (I/O error, disk full) are logged and do not crash the actor — the data is safe in the WAL file, which stays on disk and inside the flush fsync set until a retry promotes it. A dispatch that cannot reach the worker stashes its job the same way. The error itself surfaces through `PromoteWal` and `gc_checkpoint` replies and the log, and a `[promote <vol>]` warning repeats on the report cadence while any job sits stashed, naming the depth and the oldest epoch; `FLUSH` stays a pure durability barrier and keeps acking as long as its own fsyncs succeed.
...
**TRIM and WRITE_ZEROES — zero-extent path:** TRIM and WRITE_ZEROES are both implemented via zero extents (see [formats.md](formats.md) — *ZERO record*). When a filesystem (e.g. ext4) frees blocks, it issues TRIM; WRITE_ZEROES is the write-command equivalent that explicitly requires the range to read back as zeros. Both route to the same `Volume::write_zeroes()` call, which appends a ZERO WAL record per journal-window run and inserts a `ZERO_HASH` entry into the LBA map for each. A range spanning the guest's jbd2 journal window commits as boundary-aligned runs, the same split `Volume::write` takes, so each run tiers whole at formation.

Zero extents have no data payload — no hashing, no compression, no body bytes. A whole-device TRIM on a 20 GB volume writes a ~40-byte WAL record per run, and the one `mkfs.ext4` issues runs before any window is derived, so it writes exactly one. This is a significant improvement over the earlier approach of writing actual zero-filled extents via the normal write path (which required hashing the full volume worth of zero bytes).
...
The volume actor processes `gc/<ulid>.staged` files on its idle tick. For each file it: reads the staged body, walks each input's `index/<input>.idx` to derive the extent-index updates (entries present in input idx and absent from the new segment are removes; entries carried into the new segment update the extent location), updates the in-memory extent index, writes a re-signed copy to `gc/<ulid>.tmp`, and renames `<ulid>.tmp → <ulid>` (the atomic commit point), removing `<ulid>.staged`. `cache/<input>.{body,present}` are left in place for the coordinator to delete after `promote_segment` publishes the new cache body. All steps are idempotent: re-deriving the action set produces the same updates, re-deleting absent files is harmless, and re-signing produces byte-identical output. No `flush_gen` bump is needed — GC moves data between segment files only, so body offsets remain absolute.

The coordinator processes bare `gc/<ulid>` files at the start of each GC tick (`apply_done_handoffs`). For each it: uploads the body to S3; sends `promote <new-ulid>` IPC to the volume (which writes `index/<new>.idx`, `cache/<new>.{body,present}`, and deletes `index/<input>.idx` for each input — read from the new segment's header); deletes the old S3 objects; deletes `cache/<input>.{body,present}` for each consumed input directly from the filesystem; sends `finalize_gc_handoff <new-ulid>` IPC to the volume, which deletes the bare body. The coordinator never touches `index/` — every index mutation goes through the volume actor, serialised with the actor's idle-tick passes. S3 404 on delete is treated as success (idempotent across coordinator crashes).

**`index/` is fully volume-owned.** The coordinator never reads or writes `index/`. **`cache/` is append-only from the volume's perspective**: the volume creates cache entries via `promote_segment` (writes `cache/<new>.{body,present}`) and demand-fetch (appends body bytes and sets `.present` bits for a single segment), but never deletes them. All cache deletions — consumed GC inputs, coordinator-initiated eviction under disk pressure, explicit `evict` CLI — are performed by the coordinator. Keeping the writer and deleter on opposite sides of the ownership boundary makes `cache/<ulid>.body` safe to read from either process without locks: once a byte is covered by a set `.present` bit it is immutable until the file is unlinked, and unlink mid-read is benign on Linux (an open fd keeps the inode alive). Demand-fetch and normal reads continue to work whether or not the coordinator is running; eviction and GC cache cleanup pause until the coordinator returns.

**Race tolerance:** LBA map and extent index rebuild (`Volume::open`) collect `index/*.idx` paths and then read each one. If the coordinator deletes an index file during a GC cleanup pass between path collection and the read, the rebuild skips the missing file with a warning rather than failing. This is always correct: the new compacted segment (with a higher ULID) has its own `index/<new>.idx` and its entries will overwrite whatever the deleted file would have contributed. The close pass applies the same tolerance when it scans its generation directory.

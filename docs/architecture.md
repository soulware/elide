...
| `elide volume claim <name>` | Claim a `Released` name from the store: pull the released ancestor's chain, mint a fresh local fork, and conditionally rebind `names/<name>` to the new fork (state ends `Stopped`). Always CAS-protected. To override an unreachable owner of a `Live`/`Stopped` record, run `volume release --force <name>` first to flip it to `Released`, then claim |
...
**Authentication.** The coordinator authenticates volume processes via PID-bound macaroons issued on `register`, and human operators via attenuable operator tokens. The full model — caveat construction, registration and credential-exchange flows, operator-token issuance and attenuation, isolation guarantees, and credential backends — is documented in [`design-auth-model.md`](design-auth-model.md).

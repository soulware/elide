...
**Proposed: REF records with materialised body bytes.** Currently a REF record carries no data payload — the extent body lives only in the canonical segment identified by the extent index. The proposed change is to materialise the full extent body in the REF record's own segment as well, making the canonical-segment reference a *hint* rather than a load-bearing pointer.

Motivation: following a REF at S3 fetch time means a separate GET to a different S3 object — random access across objects, which is more expensive and harder to pipeline than sequential reads within a single segment. With a materialised body the fetch path is always sequential: read the body from the segment you are already fetching, ignore the ref hint entirely. The ref hint is only useful locally, when the canonical segment is already warm in `cache/`: in that case the volume can satisfy the read from the cache hit without reading the body of the current segment.
...
- **Local storage:** unchanged — the local `cache/<ulid>.body` and `pending/<ulid>` files continue to be evicted normally. The materialised body bytes are evicted alongside the rest of the segment.
- **Write path:** dedup detection is unchanged (hash → extent index lookup). On a hit, the body bytes are copied from the cached canonical extent and written into the current segment's WAL record, avoiding recompression and rehashing. The write path does not change in structure, only in that it now writes body bytes for REF records.
- **Read path:** local reads prefer the canonical cache hit (ref hint → skip body read); fall through to the materialised body in the current segment if the canonical is not cached.
- **Fetch path:** the ref hint is ignored entirely. The extent body is always present in the segment being fetched; no cross-segment GET is needed.
- **Extent index:** still required for dedup detection at write time (`hash → canonical segment`). No longer load-bearing for reads — the body is always in the current segment. The extent index entry for a REF record is still written so that future dedup checks can find the canonical location.
- **GC:** every segment now has body bytes regardless of entry type. The `has_data_entries` distinction disappears; all segments are treated uniformly by the density and sweep logic. No special-casing for dedup_ref/zero segments in GC candidate selection.

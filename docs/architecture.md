...
**Canonical ownership is "lowest non-journal ULID wins".** When the same hash appears in multiple places, the extent index makes the earliest copy canonical (first-write-wins on the live path, ULID-ascending walk at rebuild) — except that a copy whose LBA falls inside the guest filesystem's jbd2 journal window owns its hash slot only while no stable-LBA copy exists; a non-journal copy displaces it. jbd2 writes a metadata block's journal image before its home-LBA checkpoint, so without the preference the churniest LBAs on the device would own the canonicals that durable metadata DedupRefs against. With it, the home copy owns and the journal copy is the thin DedupRef that dies at wrap. The journal window is derived from the filesystem at open (`ext4_scan::journal_lba_ranges`) and persisted in `volume.toml` (`journal_ranges`) so the next open's rebuild applies the same rule before the filesystem is parseable; see `docs/design/delta-compression.md` § Journal-region awareness.

...
1. **Its own root-key keyring** — an ordered set of `(kid, key)`
   ...
   *new* artefacts. Symmetric: mint both mints and verifies under the
   ...
   only in demo mode (`[auth.demo].enabled`); a production instance
...
auto-mints a fresh keyring only when `[auth.demo].enabled`; a
...
enable [auth.demo]`) rather than silently minting one. This makes the
...
`[auth.demo]` socket (flag, else `MINT_CONFIG`), else the remembered
...
remembered at `mint login` (`auth-transport`) — the `[auth.demo]` socket
...
- **`[role.attestation]`** marks the role. Its credential is issued with
  a static attested third-party caveat at `[attestation].location` (one
...
the demo auth role (`[auth.demo]`), so `client enroll` discharges
...
colocates a demo attestation authority (`[attestation.demo]`, its own

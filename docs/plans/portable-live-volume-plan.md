...
it needs (see `docs/design/mint-volume-attestation.md` § *Recovery is a
claim* and `docs/design/force-release-fencing.md`):
...
`docs/design/mint-volume-attestation.md` § *`start` anchors on the key
shadow*):
...
2. **`volume materialize` ergonomics for chain compaction.** Already
   exists (`docs/design/replica-model.md`) but is "deferred". Frequent

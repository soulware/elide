...
`docs/design/peer-segment-fetch.md` § "Body fetch (deferred — not v1)" with
...
If those are weak, peer body fetch likely isn't worth it. If they're strong, the body-fetch design (sketched in `docs/design/peer-segment-fetch.md`) becomes the natural extension.

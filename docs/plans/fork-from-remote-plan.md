...
3. **Snapshot completeness manifest (`.manifest` file).** Proposed
   ...
   coordinator (see `docs/plans/coordinator-driven-snapshot-plan.md`),
...
7. **Duplicated ancestor prefetch on discovery.** ~~Every directory under
   ...
   `run_volume_tasks` and runs `prefetch_indexes` for itself.~~ Fixed:
   `run_volume_tasks` now skips `prefetch_indexes` for pulled ancestors
   (volumes with no `volume.name`) — the descendant writable fork's
   chain walk is the sole populator. Imported readonly bases (named,
   readonly) and writable forks still prefetch normally. See
   `tasks::is_pulled_ancestor` and the unit tests around it.

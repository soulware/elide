...
`docs/design/mint.md` (resolves open question #12). The decision stands
independent of whether Tigris can prefix-scope `ListBucket` — that is
parked (`docs/design/mint.md` #12); this work is the backend-portable answer
and a long-wanted perf win regardless.

Authority: `docs/design/mint.md` § *`coord-rw`* / open question #12.
Related: `docs/design/volume-event-log.md` (event HEAD pointer),
...
- **Per-vol "latest stable basis" — a single `by_id/<vol>/snapshots/LATEST`
  ...
  **under `volume-rw` only — no cross-role write, no event**. Migrates
  ...
  `docs/design/mint-volume-attestation.md`). Here a stale/lost pointer
...
- **`events/<name>/HEAD` — a bounded window, not a scalar pointer.**
  ...
  **Single-writer by ownership; `claim --force` is the only
  ...
  direction; `docs/design/force-release-fencing.md`). If A crashed mid-emit
  ...
  Key shape coordinated with `docs/design/volume-event-log.md`. Runs under
...
- **Snapshots / event spine have no LIST reconcile to add.** The
  ...
  `docs/design/volume-event-log.md`'s concern, not redesigned here). The
...
- **P1 — event-log spine: windowed `events/<name>/HEAD`.** Add the
  ...
  **No event-format change, no new event kinds.** Change `volume
  ...
  with `docs/design/volume-event-log.md`. Also gives the claim path its
...
- **P5 — drop the grant.** Delete `s3:ListBucket` from
  ...
  policy, and the role-inventory table in `docs/design/mint.md`. End state:
...
- Whether Tigris honours prefix-scoped `ListBucket` (`docs/design/mint.md`

...
| `volume import`/`delete` | Coordinator IPC | Safe |
| `volume status` | Direct fs (read-only); `--remote` via coordinator IPC | Safe |
| `volume inspect` | Direct fs (read-only) | Safe |
...
Snapshot-floor segments (at or below the latest snapshot ULID) are frozen and never touched. Both user snapshots (`<S>.manifest`) and stop-snapshots (`<S>-stop.manifest`) participate in the floor calculation. Stop-snapshots, however, exist only during the `Stopped` window — they are written by `volume stop` and deleted by the next `volume start` — so stop/start cycles do not progressively pin history. During the live phase the floor is determined by user snapshots only (or none); GC operates as normal. See `docs/architecture.md` *Stop-snapshot lifecycle* for the full state machine.
...
- **Eligibility.** A segment is a candidate if either holds: `live_lba_bytes == 0` (tombstone input — no live entries, no removed hashes); `density < density_threshold` and `dead_lba_bytes > 0` and has body content (sparse reclamation candidate). Snapshot-floor segments are ineligible. Size does not admit a segment: a rewrite that carries every entry through returns the same bytes under one fewer name and has to upload the replacement to do it, so packing that pays belongs to the close pass upstream of the upload.
- **Packing.** Tombstone inputs fold in for free — they contribute to the output's `inputs` list, not its body. Sparse candidates sort descending by `materialised_bytes` and first-fit into buckets bounded by a 32 MiB materialise budget *and* an 8192-entry budget — packing stops as soon as either cap would be exceeded, and an oversize input takes a bucket of its own.
- **Emit.** The pass produces one compaction plan per emitted bucket under its pre-minted ULID. A bucket of only tombstone inputs emits a zero-body tombstone handoff (see *Self-describing handoff* below).

Per-tick work is bounded by the 32 MiB materialise cap, the 8192-entry cap, the per-tick bucket cap, and O(1)-per-input tombstone bookkeeping on the apply side. "Repack-multi" falls out for free: several sparse inputs can land in one bucket. The entry cap matches the WAL flush cap (see *Pending compaction*) so GC outputs sit at the same scale as freshly-flushed segments — without it, packing many thin-entry inputs (DedupRef, Zero, small Inline) could produce a single output with an over-large index region.
...
**Idle passes.** A pass hashes its inputs before it rebuilds the liveness view. The hash covers the ordered segment list of every chain layer, the WAL files, and the snapshot floor. When it matches a previous pass that emitted no plan and deferred nothing cold, the coordinator replays that result and skips the rebuild. An idle volume's directories hold still between ticks, so an idle fork runs one real pass per `IDLE_PASS_HOLD` (120 s) rather than one per `gc_interval`. The hash costs 0.1% of the rebuild it replaces. See `docs/design/gc-idle-pass-gate.md`.

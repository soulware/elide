...
- **Packing.** Tombstone inputs fold in for free — they contribute to the output's `inputs` list, not its body. Smalls sort ascending by `live_lba_bytes` and greedy-fill a 32 MiB live budget. One filler from the sparse-large set (`live_lba_bytes > 16 MiB` AND density below threshold) may top up remaining headroom; among fitting candidates, lowest density wins, ties broken by largest `live_lba_bytes` (best fit).
...
Per-tick work is bounded by the 32 MiB live cap plus O(1)-per-input tombstone bookkeeping on the apply side. "Repack-multi" falls out for free: multiple sparse smalls can land in one bucket.

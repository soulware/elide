...
**Compression: full-extent granularity, LZ4 for body, zstd for deltas (proposed).** The lsvd reference implementation uses LZ4 for all extent compression and has no delta compression. Elide matches lsvd's granularity model: `FLAG_COMPRESSED` applies to the full stored payload; decompression on read always decompresses the complete extent, even for sub-range reads. The divergence is algorithm choice. The proposal is LZ4 for locally-written body extents (fast decompression on the hot read path — same rationale as lsvd) and zstd for delta bodies in S3 (better ratio justifies slower decompression; delta blobs are small and fetched infrequently). This two-algorithm design is an Elide-specific addition; lsvd has no delta compression at all. The current implementation uses zstd for both paths.
...
**Fetched segment format (three-file).** On a demand-fetch cache miss, the fetcher downloads the full S3 object and writes it to `fetched/` as three files: `<ulid>.idx` (header + index + inline; always written first so rebuild works on restart), `<ulid>.body` (body bytes; body-relative offsets — offset 0 = first body byte), and `<ulid>.present` (packed bitset; one bit per index entry; all bits set since the current fetcher always downloads the full body). The read path checks `fetched/<ulid>.body` before calling the fetcher, and `extentindex::rebuild` and `lbamap::rebuild_segments` scan `fetched/*.idx` at startup so evicted segments remain visible in the index. The three-file format is designed for future extent-granular fetching: individual extents can be written into the sparse `.body` file at their `body_offset` and their bit set in `.present`, without any format change. A background promotion step (planned, not yet implemented) collapses a fully-populated fetched triplet into a single `segments/<ulid>` file.
...
- **Extent-granular fetching:** the current demand-fetcher downloads the full segment body on a cache miss. The three-file fetched format is already designed for per-extent byte-range GETs (`.present` bitset, body-relative offsets in `.body`). The missing piece is the fetch-path logic to issue a GET for just the needed extent, write it at the correct body offset, and set the bit — deferring the rest of the segment body. Prioritise after eviction is working and bandwidth cost is measurable.

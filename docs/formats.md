...
**Compression granularity:** an extent over 256 KiB is stored as independently decodable frames, so a read decodes only the chunks its range lands in. Below that there is one chunk and the codec applies to the whole payload.

```
Chunked stored payload (codec 3):
  plain_length   (4 bytes)  u32 le — plaintext bytes the whole extent decodes to
  per chunk (36 bytes), ceil(plain_length / 262144) of them:

...
**Hash-dead bodies are dropped by repack before upload.** A DATA entry is *hash-dead* when its LBA no longer maps to the hash and no other live LBA anywhere in the volume references it — the body bytes can never be read again, but they would still be uploaded as part of the segment unless dropped first. Repack rewrites every pending segment with at least one hash-dead body under a freshly-minted output ULID (with `u_repack < u_flush`) and unlinks the original `pending/<input_ulid>` on apply. DedupRef entries are not touched: the thin format carries no DedupRef body bytes. Repack is idempotent — re-running it on a segment with no hash-dead bodies is a no-op.

Repack uses the same checkpoint pattern as GC and sweep: pre-mint `u_repack_i < u_flush`, flush any open WAL into `pending/<u_flush>`, then run the rewrite. The pre-mint ordering keeps every output below every future WAL ULID so concurrent writes always win on rebuild. Rewriting under a new ULID (rather than mutating in place) avoids path-aliasing a concurrent reader's pre-rewrite snapshot — the old file stays readable until apply unlinks it.
...
6. Read pending/<ULID> and stream it byte-for-byte to S3.
   The local file is already in its final form by the time drain runs:
     ...
     - Hash-dead DATA entries have been dropped by repack — segments
       with any hash-dead body have been rewritten under a fresh
       `u_repack < u_flush`.
...
**Codec byte** (1 byte per entry): `0` raw, `1` lz4_flex with a `u32` le uncompressed-length prefix, `2` a single zstd frame, `3` a chunk table followed by one zstd frame per 128 KiB of plaintext. A codec byte the reader does not know fails the parse, so stored bytes are never handed back as plaintext by default. The byte names the codec of the uploaded body; a read takes its codec from the location it resolved. Every codec declares its plaintext length before decoding — lz4 in its prefix, zstd in its frame header, raw by being the plaintext — and a length over `segment::MAX_EXTENT_PLAINTEXT` is refused, since the body section is unsigned and a decoder sizes its output from that number.

Durable bodies take zstd: body bytes are what a segment uploads and what S3 charges for, and `volume::compress_body` keeps the compressed form whenever it is smaller than the plaintext. Journal-tier bodies take lz4, as does the inline section. The WAL and `.dmat` take lz4 through `volume::maybe_compress`, which keeps the compressed form only when it is at least 1.5× smaller — a bar that prices decompression CPU on a latency path. Incompressible data fails that ratio naturally, so lz4 itself decides where a precomputed entropy gate would. See [compression-codecs.md](design/compression-codecs.md) for the codec each artefact takes and why.
...
**Compression granularity:** an extent over 128 KiB is stored as independently decodable frames, so a read decodes only the chunks its range lands in. Below that there is one chunk and the codec applies to the whole payload.

```
Chunked stored payload (codec 3):
  plain_length   (4 bytes)  u32 le — plaintext bytes the whole extent decodes to
  per chunk (36 bytes), ceil(plain_length / 131072) of them:
    stored_length (4 bytes)  u32 le — compressed bytes of this chunk
    cv            (32 bytes) BLAKE3 chaining value of this chunk's plaintext
  then the chunk frames, concatenated in index order
```

Chunk offsets are the prefix sum of the stored lengths, so they are not stored. The table sits in the body section rather than the index because `.idx` is fetched eagerly for every ancestor at open, and a volume that never reads a body should not carry its chaining values.

**Chunk verification.** 128 KiB is a power-of-two multiple of BLAKE3's own 1 KiB chunk, so a slice of the plaintext at a multiple of it is a subtree of the extent's hash tree and the stored `cv` is that subtree's chaining value. A reader decodes the chunks it needs, substitutes their computed chaining values into the table's, reconstructs the root, and compares it to the extent hash the signed index already carries. A wrong chunk gives a wrong root, and so does a tampered chaining value for any chunk the reader did not decode, so one comparison covers the decoded bytes and the whole unsigned table. The table needs no signature and no anchor of its own. See `elide-core/src/chunk_tree.rs`.
...
Hash-dead DATA entries (LBA overwritten and hash no longer referenced by any live LBA in the volume) are dropped by repack before upload — the segment is rewritten under a freshly-minted `u_repack < u_flush` with the dead bodies omitted, and the original `pending/<input_ulid>` is unlinked on apply. Deleted data is therefore never transmitted to S3. DedupRef entries are skipped by repack — they contribute zero body bytes in the thin format.

...
- **Concurrency with the owner.** The owner may publish a new snapshot, GC, or repack while `volume fetch` is running. Steps 4–6 are linear and operate on a captured manifest, so a mid-run owner publish is benign — the new manifest will be picked up on the next `volume fetch` run. A concurrent GC that deletes a segment between manifest fetch and body fetch presents as a 404 on the body GET. Existing demand-fetch handling treats this as a transient failure; whether `volume fetch` should retry against a fresh manifest or surface the partial result is a policy question.

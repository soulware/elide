...
- **Mid-session journal-window flip.** Every proptest fork stamps a `[journal]` window before its first open, so the never-derived poll (`poll_derive_and_flip`) and the live flip with its activation marker run only in the `volume/tests.rs` unit tests (`format_mid_session_flips_window_at_promote_take`). A dedicated flip proptest (format op + window/stable writes + crash + local GC, byte oracle) wants the synthetic ext4 image builder (`ext4_scan::test_support`, currently `#[cfg(test)] pub(crate)`) reachable from the integration-test crate, and journal extents relocated clear of the shared op set's LBA space.
...
## Coordinator ownership + fork-rehome proptest

`elide-coordinator/tests/portable_proptest.rs` races **two coordinators** over a shared `names/<name>` registry on one in-memory bucket, each with its own identity and `data_dir`. Ops (`Create`, `Release`, `ForceClaim`, `ClaimReleased`, `ReclaimLocal`, `Tick`) drive the real lifecycle CAS path (`mark_claimed_force`, `mark_claimed`, …); the model materializes each coordinator's local fork layout (`by_id/<ulid>` + `by_name` symlink, plus the `volume.released` marker a release stamps and an in-place reclaim clears) so a takeover leaves the previous owner holding a stale local binding.

`Op::Tick` is the ownership poll: for each fork a coordinator still holds it re-reads `names/<name>`, and on a `vol_ulid` mismatch calls the real `rehome::rehome_displaced_fork` (see `docs/design/displaced-fork-rehome.md`), then re-invokes it asserting the same result — the crash-retry walking the episode-derived candidate sequence back to its own record. Both dispositions are exercised: a force-claimed fork rehomes as a displacement, a released-then-claimed one as a supersession. Invariants checked against the real bucket after every op: a rehomed fork's `<name>-<suffix>` record is `Released`, ownerless (release shape), and binds the old ULID (preserve-never-orphan), with any handoff it carries verifying like a release's; no two `Live` records share a `vol_ulid` (single-writer-per-name); a rehomed coordinator stops serving the lost fork; and re-observation resolves to the same name (idempotent rehome).

//! ObjectStore-backed fork-ancestry walker for the peer-fetch auth pipeline.
//!
...
//! its fork-parent prefixes — so the walk is fork-only. (Coord B's
//! attestation walk, which *does* union `extent_index` sources, lives
//! in the `elide-attestation` crate.)

...
| [docs/design-iam-key-model.md](docs/design-iam-key-model.md) | Superseded by design-mint.md (coordinator in-process path + `[iam]` section removed); retained for the key-inventory/policy-scoping rationale mint's roles inherit. Per-volume IAM key model for Tigris-style backends. Four key classes (admin, one writer per coordinator, one peer-fetch per coordinator, per-volume RO with ancestor-inclusive policies) plus ephemeral fetch keys for `volume fetch`. IAM-layer invariants: `events/` is append-only and `coordinators/` is immutable (no key holds Delete). Identity via policy names (Tigris doesn't tag keys); host-local reconciliation; dead-host orphans need operator action |
...
| [docs/design-domain-store.md](docs/design-domain-store.md) | Proposed: replace `Arc<dyn ObjectStore>` (215 occurrences across 23 files in `elide-coordinator/src/`) with object-typed handles vended by role — `NameClaims`, `EventJournal`, `OwnIdentity`, `VolumeData` (segments/snapshots/retention/head/metadata sub-accessors), `ControlPlaneReader`. S3 key layout collapses into one module so wrong-prefix keys become unconstructable; `events/` append-only invariant becomes type-level (no `delete` method). Migrates per-object alongside `ScopedStores`, no atomic swap |

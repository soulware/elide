...
| [docs/design-volume-size-ownership.md](docs/design-volume-size-ownership.md) | Proposed: relocate `size` from unsigned `manifest.toml` to the `names/<name>` claim record (single owner, CAS-protected, signed in event log). Ancestors stop carrying size at all; resize becomes a CAS + `UBLK_U_CMD_UPDATE_SIZE` (Linux 6.16+) |

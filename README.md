...
| [docs/coordinator-mint-enrollment-plan-v2.md](docs/coordinator-mint-enrollment-plan-v2.md) | Plan: coordinator-side mint enrollment — one blocking `elide coord enroll` (`A → wait approval → exchange fan-out`) writing `credentials/<role>`, plus a hard `[mint]` startup gate. Threads the three operator-discharge gates (request / initialize / admin); needs a logged-in operator session; ticket in-memory only; bootstrap operator-supplied not config. Supersedes the v1 plan. |

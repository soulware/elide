...
| [docs/coordinator-mint-enrollment-plan.md](docs/coordinator-mint-enrollment-plan.md) | Plan: coordinator-side mint enrollment — one blocking `elide coord enroll` (`A → wait approval → exchange fan-out`) writing `credentials/<role>`, plus a hard `[mint]` startup gate. Ticket in-memory only; bootstrap operator-supplied not config. |

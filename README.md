...
| [docs/design-auth-model.md](docs/design-auth-model.md) | Auth model: operator tokens (coord-wide, IPC-minted via `elide token create`, CLI-side attenuation per use, 30d default expiry), isolation guarantees on a shared-uid host, audit log structure. References `architecture.md` for the underlying macaroon construction (chained MAC + per-token nonce) shared with volume macaroons |
...
| [docs/design-mint-template-seal.md](docs/design-mint-template-seal.md) | Proposed: signed seal (`_mint/templates/seal.json`, MAC'd under the keyring) over role TOML blocks + BLAKE3 hashes of policy template files. `mint seal` is the single explicit operator command that publishes a seal; startup verifies and refuses-closed on mismatch; in-memory cache for the process lifetime (no per-render disk work). Templates themselves stay operator-provisioned out-of-band; the seal carries only their hashes. No hot-reload, no auto-seal-on-start — one possible running state |

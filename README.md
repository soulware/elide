...
| [docs/design-auth-model.md](docs/design-auth-model.md) | Auth model and principle: PoC operator tokens (coord-wide, IPC-minted via `elide token create`, CLI-side attenuation per use, 30d default expiry), isolation guarantees on a shared-uid host, audit log structure; settled principle that operator tokens gate S3 *writes* not destructive verbs. References `architecture.md` for the keyed-BLAKE3 macaroon construction shared with volume macaroons. The concrete central-auth-service design lives in design-auth-service.md |
| [docs/design-auth-service.md](docs/design-auth-service.md) | Proposed: central auth service issues operator session + per-op discharge macaroons, replacing the local operator-token PoC. Multi-tenant by construction (mandatory `OrgId` caveat; mint is per-org and the org-identity broker; coord enrolls to a mint, not direct to auth service). Login via device-code (SSH-friendly) or `ELIDE_OPERATOR_API_KEY` for automation. Ed25519 signatures for the auth-service-issued chain. Mint-as-auth packaging for dev/test/demo via `demo-enabled = true`. Concrete `/v1/` API surface for login, discharge, mint enrollment, and coord enrollment. Clean-break migration from PoC |

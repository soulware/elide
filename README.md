...
| [docs/design-gc-bucket-unification.md](docs/design-gc-bucket-unification.md) | Proposed: collapse GC's smalls + one-filler tiers into a single bin-pack that emits N output buckets per tick; mirrors #297's pending unification, gated behind a `max_buckets_per_tick` knob (default 1) so rollout is behaviour-preserving |

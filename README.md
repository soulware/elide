...
| [docs/design-iam-key-model.md](docs/design-iam-key-model.md) | Proposed: per-volume IAM key model for Tigris-style backends. Four key classes (admin, one writer per coordinator, one peer-fetch per coordinator, per-volume RO with ancestor-inclusive policies) plus ephemeral fetch keys for `volume fetch`. IAM-layer invariants: `events/` is append-only and `coordinators/` is immutable (no key holds Delete). Identity via policy names (Tigris doesn't tag keys); host-local reconciliation; dead-host orphans need operator action |
| [docs/design-auth-model.md](docs/design-auth-model.md) | Auth model: macaroon construction (chained MAC, attenuable), volume macaroons (PID-bound, registration + credential exchange), operator tokens (coord-wide, IPC-minted via `elide token create`, CLI-side attenuation per use, 30d default expiry), isolation model, credential backends |

...
pub enum Request {
    ...
    /// [`Request::MintOperatorToken`] and
    /// `docs/design-auth-model.md`). The CLI attaches
    /// `Op(Remove)`, `Volume(volume)`, and a short `NotAfter` to its
    /// stored token before sending it here. Absent or invalid →
    /// `Envelope::Err { kind: "forbidden" }`.
    Remove {
        volume: String,
        #[serde(default)]
        force: bool,
    ...
    /// Mint a coordinator-wide operator token. The CLI calls this
    /// when the operator runs `elide token create`. The minted token
    /// carries `Role::Operator`, a random `Nonce`, and a `NotAfter`
    /// at `expires_unix`. It does *not* carry `Op` or `Volume` — the
    /// CLI narrows those per use via attenuation before presenting
    /// the token to a gated verb.
    ///
...
/// Reply for [`Request::MintOperatorToken`]. The encoded macaroon
/// is what the operator stores at `~/.elide/operator-token` (or
/// passes via `--token` / `ELIDE_OPERATOR_TOKEN`). `nonce_hex` and

...
async fn attested_loop_over_shipped_templates() {
    ...
    // `roles/` is generated by `mint render` (run after the config is written,
    // below); the shipped templates live in role-templates/. The store is local,
    // so the rendered bucket name is cosmetic; any valid name works.
    let rendered_roles = root_p.join("roles");
    {
        let tbl = cfg_doc.as_table_mut().expect("config table");
        {
            ...
            set(
                "template_dir",
                deploy.join("role-templates").display().to_string(),
            );
    ...
    // `mint render` reads template_dir + roles_dir from the config and resolves
    // the `{{build.bucket}}` token before mint seals them. Running the real
    // command keeps the e2e in lockstep with the documented deploy flow.

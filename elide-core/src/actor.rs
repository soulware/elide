// VolumeActor + VolumeClient: the intended integration pattern for ublk and NBD.
//
...
// reads, writes, flushes, and control operations. Every clone shares the
// same mailbox, the same atomic ReadSnapshot pointer, and the same volume-
// level file-descriptor cache.
//
// Reads bypass the channel entirely: the client loads the current
...
/// `VolumeClient` clones without copying.
...
///
/// `Send + Sync + Clone`. Clones share:
/// - The mailbox sender (writes go through the actor).
/// - The atomic snapshot pointer (reads resolve against it lock-free).
/// - A volume-level LRU cache of open segment file handles, used by `read`.
///
/// The file cache is shared across all clones — it is a volume-scoped
/// resource (one hot set of segments per volume regardless of how many
/// threads are serving I/O), not a per-thread resource. Concurrent reads
/// hold the cache mutex only for the lookup/insert step; the `pread(2)`
/// that follows runs on a cloned `Arc<File>` with the lock released.
#[derive(Clone)]
pub struct VolumeClient {
    tx: Sender<VolumeRequest>,
    snapshot: Arc<ArcSwap<ReadSnapshot>>,
    config: Arc<VolumeConfig>,
    /// Volume-level file-descriptor cache for the read path. Shared across
    /// every clone of this client. See [`FileCache`](crate::volume::FileCache).
    ...
    /// on every read; if they differ the cache is cleared before proceeding.
    last_flush_gen: Arc<std::sync::atomic::AtomicU64>,
}
...
impl VolumeClient {
    ...
    ///
    /// Thread-safe: the shared file cache is a `Mutex<FileCache>` held only
    /// for lookup/insert; the `pread(2)` runs on a cloned `Arc<File>` with the
    /// lock released.
    pub fn read(&self, lba: u64, lba_count: u32) -> io::Result<Vec<u8>> {
        ...
        let snap = self.snapshot.load();
        let last = self.last_flush_gen.load(Ordering::Acquire);
        if snap.flush_gen != last
            && self
                .last_flush_gen
                .compare_exchange(last, snap.flush_gen, Ordering::AcqRel, Ordering::Acquire)
                .is_ok()
        {
            // Only the thread that won the CAS clears the cache. Other threads
            // see the updated generation on the next `read` via Acquire.
            self.file_cache.lock().expect("file cache poisoned").clear();
        }
...
/// `VolumeClient` can be cloned freely (`Send + Sync + Clone`); every clone
/// shares the mailbox, the atomic snapshot pointer, and the volume-level
/// file-FD cache used by `read`.
///
...
pub fn spawn(volume: Volume) -> (VolumeActor, VolumeClient) {
    ...
    let client = VolumeClient {
        ...
        snapshot,
        config,
        file_cache: Arc::new(std::sync::Mutex::new(crate::volume::FileCache::default())),
        last_flush_gen: Arc::new(std::sync::atomic::AtomicU64::new(0)),
    };
